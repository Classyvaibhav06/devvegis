    ...
    <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
      {categories.map((cat) => (
        <Link key={cat.slug} href={`/categories/${cat.slug}`}>
          <div className="flex flex-col items-center text-center p-3.5 rounded-2xl bg-white dark:bg-[#0F1520] border border-slate-200/80 dark:border-white/[0.07] hover:border-emerald-500/40 group cursor-pointer h-full transition-all duration-200 relative overflow-hidden shadow-xs hover:shadow-md dark:shadow-lg dark:hover:shadow-emerald-500/5">
